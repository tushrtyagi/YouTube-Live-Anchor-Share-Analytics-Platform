...
shutil.rmtree('../Output_Screen_Grab_YT_YT/YT_Screen/', ignore_errors=True)
if not os.path.exists(f"../Output_Screen_Grab_YT/YT_Screen"):
    os.makedirs(f"../Output_Screen_Grab_YT/YT_Screen")
...
def extract_frames(video_path, output_path, interval=30):
    ...
    while success:
        ...
        if current_time >= frame_count * interval:
            ...
            # SPAR Group
            result = greenAPI.sending.sendFileByUpload('120363234411475039@g.us',
                                                       output_path + frame_name,
                                                       output_path + frame_name, "")
            print(result.data)
            print('???????????????????????', output_path + frame_name)
...
def get_filename(df, channel_name):
    ...
    port_dict = {
        "Aaj Tak-YT": str(path) + "1_TS-7_1",
        "ABP News-YT": str(path) + "1_TS-13_203",
        "TV9 Bharatvarsh-YT": str(path) + "1_TS-18_335",
        "India TV-YT": str(path) + "1_TS-14_201",
        "Zee News-YT": str(path) + "1_TS-25_204",
        "Times Now Navbharat-YT": str(path) + "1_TS-16_207",
        "Republic Bharat-YT": str(path) + "1_TS-17_2163",
        "News18 India-YT": str(path) + "1_TS-15_202",
        "Tez-YT": "Logger02Port02",
        "NDTV India-YT": str(path) + "1_TS-19_205",
        "India Today TV - YT": str(path) + "1_TS-8_2",
        "Republic 24x7 - YT": str(path) + "1_TS-20_23",
        "Times Now - YT": str(path) + "1_TS-24_301",
        "CNN News18 - YT": str(path) + "1_TS-23_302"
    }

    # Dictionary for Channel name and its folder location in logger
    portname_dict = {
        "Aaj Tak-YT": "1_TS-7_1",
        "ABP News-YT": "1_TS-13_203",
        "TV9 Bharatvarsh-YT": "1_TS-18_335",
        "India TV-YT": "1_TS-14_201",
        "Zee News-YT": "1_TS-25_204",
        "Times Now Navbharat-YT": "1_TS-16_207",
        "Republic Bharat-YT": "1_TS-17_2163",
        "News18 India-YT": "1_TS-15_202",
        "Tez-YT": "Logger02Port02",
        "NDTV India-YT": "1_TS-19_205",
        "India Today TV - YT": "1_TS-8_2",
        "Republic 24x7 - YT": "1_TS-20_23",
        "Times Now - YT": "1_TS-24_301",
        "CNN News18 - YT": "1_TS-23_302"
    }
...
def arr(language):
    ...
    if language == "English":
        src_path = "../Input Files/Helpers/English"
        all_csv = glob.glob(src_path+"/*.csv" )
    if language == "Hindi":
        src_path = "../Input Files/Helpers/Hindi"
        all_csv = glob.glob(src_path+"/*.csv" )
...
'''
            ...
            result = greenAPI.sending.sendMessage(
                '120363234411475039@g.us', text1)  # ITGGROUP
            # import pdb;pdb.set_trace()
            # result = greenAPI.sending.sendMessage('918178139556@g.us', text1)  # Nilotpal Kumar
            ...
            print('....................', channel_name)
            if not os.path.exists(f"../Output_Screen_Grab_YT/YT_Screen/{channel_name}"):
                os.makedirs(
                    f"../Output_Screen_Grab_YT/YT_Screen/{channel_name}")
...
for i in sorted(glob.glob("../Input Files\\YT_Input_Files\\*.xlsx"), key=len):
    print("Filename>>>>>>>>>>>>>>>>>", i)
    week = i.split('-')[-1].replace('.xlsx', '').strip().split("'")[0]
    year = str(20) + \
        i.split('-')[-1].replace('.xlsx', '').strip().split("'")[1]
    ...
    # Identify the channel columns (they contain '-YT' in the name)
    channel_columns = [col for col in data.columns if str(col).endswith("YT")]
    ...
    if language == "Hindi":
        ...
        # top_channels[4] = "News18 India-YT"
    elif language == "English":
        # Define the specific English channels

        english_channels = ["India Today TV - YT",
                            "Republic 24x7 - YT", "CNN News18 - YT"]
    ...
    for rank, top_channel in enumerate(top_channels_ordered, start=1):
        ...
        if language == "English":
            top_20_rows.to_csv(
                f"../Input Files/Helpers/English/{rank}{rank_suffix}{language}.csv")
        if language == "Hindi":
            top_20_rows.to_csv(
                f"../Input Files/Helpers/Hindi/{rank}{rank_suffix}{language}.csv")
